...
var format_date = R.compose
  ( R.map(s => s.split('-').reverse().join('/'))
  , date_only
  , R.map(d => date_to_string((new Date(d)).setHours(12))) // hack because of weirdness with times, sails and waterline
  , check_date // allows arg to be date string
  )
