  ...
  render: function () {
    ...
    return (
      <div className={'table ' + this.props.className}>
        <TableHeader header_row entries={headers} headers={headers}/>
        <div className='table-body' ref='tbody'>
          {rows.map((row, i) => <TableRow {...this.props} entries={row} headers={headers} header_row={false} row_num={i} key={i}/>)}
        </div>
      </div>
    )
  }
