...
function get_description (payment) {
  return payment.description + (payment.type
    ? ' - ' + payment.type
    : '')
}

var PaymentsTable = ({remove_payment, payments = []}) => {
  var headers = ['Date', 'Description', 'Charges', 'Payments', 'Balance Due'].concat(remove_payment
    ? ['Reference', 'Notes', 'Delete']
    : []
  )

  var entries = make_payments_with_balance(payments).slice(1).map((payment) => {
